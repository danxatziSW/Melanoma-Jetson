...
TRAIN_2  = "isic2020"
AUG_MODE = "none"
SENS_ID  = "none_sens"   # suffix used by train_sensitivity_pair.py

ALL_DATASETS = ["ham10000", "isic2019", "isic2020"]

# wider sweep: fine-tuned models output higher probs, old ceiling of 0.65 was too low
...
def main() -> None:
    ...
    for ds in ALL_DATASETS:
        ...
        ds_aucs[ds]  = auc
        print(f"    AUC = {auc:.4f}  "
              f"(prob range: {mean_p.min():.3f} – {mean_p.max():.3f}  "
    ...
    for i, (col, desc) in enumerate([
        ...
        ("Yellow", "Best F2 — maximises avg F2 (β=2), weights sensitivity 2×"),
