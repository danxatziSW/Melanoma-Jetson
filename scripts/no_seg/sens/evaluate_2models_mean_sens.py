...
def run_group(
    ...
    print(f"\n    Evaluating {n_pairs} pairs from {len(available)} checkpoints ...")
    ...
    for k1, k2 in combinations(available, 2):
        ...
        thr_f1 = _best_threshold_f1(pair_ds_probs)
        thr_f2 = _best_threshold_f2(pair_ds_probs)
        ...
        summary["avg_sensitivity"] = round(float(np.mean(
            [summary[f"sensitivity_{ds}"] for ds in eval_datasets if f"sensitivity_{ds}" in summary])), 4)
...
def main() -> None:
    ...
    eval_xlsx         = ablation_noseg_dir / "evaluation_2models_mean_sens.xlsx"
    ...
    print(f"  Monitors      : F1 and F2 (both optimised separately)\n")
    ...
    for eval_ds in ALL_DATASETS:
        if f"f1_{eval_ds}" not in df.columns:
            continue
        ds_id   = ["pair_name", "model_1", "train_1", "model_2", "train_2",
                   "best_threshold_f1", "best_threshold_f2"]
        ...
        write_excel_sheet(eval_xlsx, eval_ds.upper(),
                          ...
                          .sort_values(f"f1_{eval_ds}", ascending=False))

    best = (df.groupby("pair_name", as_index=False)
    ...
    pair_info = df.drop_duplicates("pair_name")[
        ["pair_name", "model_1", "train_1", "model_2", "train_2",
         "best_threshold_f1", "best_threshold_f2"]]
    best = best.merge(pair_info, on="pair_name", how="left")
    best_cols = ["pair_name", "model_1", "train_1", "model_2", "train_2",
                 "best_threshold_f1", "best_threshold_f2",
                 "avg_f1", "avg_f2", "avg_sensitivity", "avg_specificity",
                 "avg_accuracy", "avg_sens_f2"]
    write_excel_sheet(eval_xlsx, "Best_Pairs_F1",
                      best[[c for c in best_cols if c in best.columns]])
    write_excel_sheet(eval_xlsx, "Best_Pairs_F2",
                      best[[c for c in best_cols if c in best.columns]]
                      .sort_values("avg_f2", ascending=False))

    best_pair_f1 = df.loc[df["avg_f1"].idxmax()]
    best_pair_f2 = df.loc[df["avg_f2"].idxmax()]

    sep = "=" * 80
    print(f"\n{sep}")
    print(f"  Saved : {eval_xlsx}")
    print(f"  Pairs : {len(df)}  (from {len(all_singles)} loaded checkpoints)")
    print(f"\n  ── Best pair by F1 (thr={best_pair_f1['best_threshold_f1']:.2f}) ──")
    print(f"     {best_pair_f1['pair_name']}")
    print(f"     avg_f1={best_pair_f1['avg_f1']:.4f}  avg_f2={best_pair_f1['avg_f2']:.4f}  "
          f"avg_sens={best_pair_f1['avg_sensitivity']:.4f}  avg_spec={best_pair_f1['avg_specificity']:.4f}")
    for ds in ALL_DATASETS:
        if f"f1_{ds}" in best_pair_f1:
            print(f"     {ds.upper():<12} F1={best_pair_f1[f'f1_{ds}']:.4f}  "
                  f"sens={best_pair_f1[f'sensitivity_{ds}']:.4f}  "
                  f"spec={best_pair_f1[f'specificity_{ds}']:.4f}")
    print(f"\n  ── Best pair by F2 (thr={best_pair_f2['best_threshold_f2']:.2f}) ──")
    print(f"     {best_pair_f2['pair_name']}")
    print(f"     avg_f1={best_pair_f2['avg_f1']:.4f}  avg_f2={best_pair_f2['avg_f2']:.4f}  "
          f"avg_sens={best_pair_f2['avg_sens_f2']:.4f}  avg_spec={best_pair_f2['avg_spec_f2']:.4f}")
    for ds in ALL_DATASETS:
        if f"f2_{ds}" in best_pair_f2:
            print(f"     {ds.upper():<12} F2={best_pair_f2[f'f2_{ds}']:.4f}  "
                  f"sens={best_pair_f2.get(f'sens_f2_{ds}', float('nan')):.4f}  "
                  f"spec={best_pair_f2.get(f'spec_f2_{ds}', float('nan')):.4f}")

    print(f"\n  Top 5 by F2:\n")
    top5 = df.sort_values("avg_f2", ascending=False).head(5)
    print(top5[["pair_name", "best_threshold_f2", "avg_f1", "avg_f2",
                "avg_sensitivity", "avg_specificity", "avg_accuracy"]].to_string(index=False))
    print(f"{sep}\n")
