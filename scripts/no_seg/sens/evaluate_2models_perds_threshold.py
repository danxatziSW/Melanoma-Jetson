...
def _best_f2_thr_single(probs: np.ndarray, labels: np.ndarray) -> tuple[float, dict]:
    """Find threshold maximising F2 for ONE dataset independently."""
...
def run_group(
    ...
    for k1, k2 in combinations(available, 2):
        ...
        summary["min_sensitivity"] = round(float(np.min(per_sens)),  4)   # worst-case dataset
...
def main() -> None:
    ...
    print(f"  Threshold rng : {THRESHOLDS[0]} – {THRESHOLDS[-1]}\n")
