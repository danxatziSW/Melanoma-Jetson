# all c(n,3) triplets, mean probs + shared threshold instead of majority vote. aug=none only. no-seg version
...
DEFAULT_THRESHOLD = 0.50   # reference threshold -- saved to quantify gain from tuning
...
def run_group(
    ...
    if len(checkpoint_probs) < 3:
        print("    [SKIP] fewer than 3 checkpoints loaded, cannot build ensembles")
    ...
    for key, ds_dict in checkpoint_probs.items():
        ...
        _add_thr50_metrics(row, {ds: (p, l) for ds, (p, l, _) in ds_dict.items()})
        single_results.append(row)

    available = list(checkpoint_probs.keys())
    n         = len(available)
    n_combos  = len(list(combinations(available, 3)))
    print(f"    Evaluating {n_combos} combos from {n} checkpoints (mean prob + threshold sweep) ...")
    ...
    for combo in combinations(available, 3):
        ...
        mean_thr = _best_threshold(combo_ds_probs)
        ...
        _add_thr50_metrics(summary, combo_ds_probs)
...
def main() -> None:
    ...
    eval_xlsx    = noseg_dir / "evaluation_3models_mean.xlsx"
    ...
    print(f"  Combos (max)         : {n_combos_est}  (C({n_ckpts},3))")
    print(f"  Method               : mean probability + best shared threshold per combo\n")
    ...
    id_cols = [
        ...
        "avg_f1_thr50", "avg_sensitivity_thr50", "avg_specificity_thr50", "delta_avg_f1",
    ]
    ...
    if all_singles:
        ...
        single_id  = ["model", "train_dataset", "best_threshold",
                      "avg_f1", "avg_sensitivity", "avg_specificity", "avg_accuracy",
                      "avg_f1_thr50", "avg_sensitivity_thr50", "avg_specificity_thr50", "delta_avg_f1"]
    ...
    for eval_ds in ALL_DATASETS:
        ...
        ev_df = df[[c for c in ds_cols if c in df.columns]].sort_values(f1_col, ascending=False)
    ...
    best_cols = ["combo_name", "model_1", "train_1", "model_2", "train_2", "model_3", "train_3",
                 "mean_threshold", "avg_f1", "avg_accuracy", "avg_sensitivity", "avg_specificity"]
    write_excel_sheet(eval_xlsx, "Best_Combos", best[[c for c in best_cols if c in best.columns]])

    best_combo  = df.loc[df["avg_f1"].idxmax()]
    best_single = max(all_singles, key=lambda r: r["avg_f1"]) if all_singles else None

    sep = "=" * 80
    print(f"\n{sep}")
    print(f"  Results saved : {eval_xlsx}")
    print(f"  Sheets        : Summary | Single_Models | Best_Combos | {' | '.join(ds.upper() for ds in ALL_DATASETS)}")
    print(f"  Total combos  : {len(df)}")

    print(f"\n  Best 3-model mean ensemble (no-seg):")
    print(f"    Combo     : {best_combo['combo_name']}")
    print(f"    Threshold : {best_combo['mean_threshold']:.2f}")
    print(f"    avg_f1    : {best_combo['avg_f1']:.4f}  avg_accuracy : {best_combo.get('avg_accuracy', float('nan')):.4f}")
    for eval_ds in ALL_DATASETS:
        if f"f1_{eval_ds}" in best_combo:
            print(
                f"    {eval_ds.upper():<12}  "
                f"AUC={best_combo.get(f'auc_{eval_ds}', float('nan')):.4f}  "
                f"F1={best_combo[f'f1_{eval_ds}']:.4f}  "
                f"acc={best_combo.get(f'accuracy_{eval_ds}', float('nan')):.4f}  "
                f"sens={best_combo[f'sensitivity_{eval_ds}']:.4f}  "
                f"spec={best_combo[f'specificity_{eval_ds}']:.4f}"
            )

    if best_single:
        delta = round(float(best_combo["avg_f1"]) - float(best_single["avg_f1"]), 4)
    ...
    print(f"\n  Top 5 combos:\n")
    print(df.head(5)[["combo_name", "mean_threshold", "avg_f1", "avg_accuracy",
                       "avg_f1_thr50", "delta_avg_f1",
                       "avg_sensitivity", "avg_specificity"]].to_string(index=False))
    print(f"{sep}\n")
